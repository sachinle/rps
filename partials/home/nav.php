...
<header id="rps-nav" class="fixed top-0 inset-x-0 z-[1000] px-4 md:px-8 py-2">
    <div class="mx-auto flex max-w-[1320px] items-center justify-between gap-4">
        ...
        <nav class="hidden lg:flex items-center gap-7 text-sm font-semibold text-slate-700">
            <?php foreach ($menu as $label => $href): ?>
                <a href="<?= $href ?>" class="nav-link transition-colors hover:text-rps-pink"><?= $label ?></a>
            <?php endforeach; ?>
        </nav>

        <div class="hidden lg:flex items-center gap-3">
            <a href="tel:+918144260260" class="flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-rps-pink transition-colors">
            ...
            <a href="#admission" class="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-rps-pink to-rps-pinkdark px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-rps-pink/30 transition hover:scale-105">
                Apply Now
    ...
    <div id="rps-mobile" class="hidden lg:hidden mt-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-xl">
        <nav class="flex flex-col gap-1 text-base font-semibold text-slate-700">
            <?php foreach ($menu as $label => $href): ?>
                <a href="<?= $href ?>" class="rounded-lg px-3 py-2.5 hover:bg-slate-100 hover:text-rps-pink transition-colors"><?= $label ?></a>
            <?php endforeach; ?>
            <a href="#admission" class="mt-2 inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-rps-pink to-rps-pinkdark px-5 py-3 font-semibold text-white">Apply Now</a>
        </nav>
    </div>
</header>
